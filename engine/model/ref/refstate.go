...
// provides the [GuardedStates] reference for
...
// GuardedStates maps each key K to a state V advanced by transitions
// M. Construct with [NewGuardedStates]. Thread-safe.
type GuardedStates[K comparable, V, M any] struct {
	mu       sync.Mutex
	states   map[K]V
	initial  V
...
// NewGuardedStates constructs a [GuardedStates] machine.
//
...
func NewGuardedStates[K comparable, V, M any](
	initial V,
	guard func(from V, m M) (to V, ok bool),
	rejected error,
) *GuardedStates[K, V, M] {
	return &GuardedStates[K, V, M]{
		states:   make(map[K]V),
		initial:  initial,
		guard:    guard,
		rejected: rejected,
...
// error is returned.
func (c *GuardedStates[K, V, M]) Apply(_ context.Context, k K, m M) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	cur, ok := c.states[k]
	if !ok {
...
func (c *GuardedStates[K, V, M]) State(_ context.Context, k K) (V, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	s, ok := c.states[k]
	if !ok {
