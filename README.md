...
Pre-1.0. The runtime primitives (`MethodStub[T]`, `Recorder[T]`, `FaultInjector`, `StartContract`, golden-file helpers) and the generator engine (`gen/`) are stable. Generator vocabulary and directive semantics may change in minor versions until the V1 cut. Consumers should pin and regenerate on upgrade.
