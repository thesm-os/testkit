...
func TestRetryUntil(t *testing.T) {
	...
	t.Run("succeeds when pred returns true immediately", func(t *testing.T) {
		t.Parallel()
		polling.RetryUntil(t, time.Second, func() bool { return true }, "must pass")
	})
	...
	t.Run("fatals on timeout", func(t *testing.T) {
		...
		polling.RetryUntil(f, 50*time.Millisecond, func() bool { return false }, "never succeeds")
...
func TestAssertEventually(t *testing.T) {
	...
	t.Run("passes when assertion succeeds immediately", func(t *testing.T) {
		t.Parallel()
		polling.AssertEventually(t, time.Second, 10*time.Millisecond, func(tb testing.TB) {
			tb.Helper()
	...
	t.Run("fatals on timeout with last failure message", func(t *testing.T) {
		t.Parallel()
		f := testkit.NewFailableTB()
		polling.AssertEventually(f, 50*time.Millisecond, 10*time.Millisecond, func(tb testing.TB) {
			tb.Helper()
