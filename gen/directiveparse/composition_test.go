...
func TestValidateComposition(t *testing.T) {
	...
	t.Run("no directives has no issues", func(t *testing.T) {
		t.Parallel()
		issues := directiveparse.ValidateComposition(nil)
	...
	t.Run("independent directives compose cleanly", func(t *testing.T) {
		...
		issues := directiveparse.ValidateComposition(dirs)
	...
	t.Run("pure conflicts with sideeffect", func(t *testing.T) {
		...
		testkit.Equal(t, issues[0].Kind, directiveparse.Conflict, "must be conflict")
	...
	t.Run("retry-succeeds-on-attempt requires retryable", func(t *testing.T) {
		...
		testkit.Equal(t, issues[0].Kind, directiveparse.MissingRequired, "must be missing-required")
	...
	t.Run("pure with idempotent is redundant", func(t *testing.T) {
		...
		testkit.Equal(t, issues[0].Kind, directiveparse.Redundant, "must be redundant")
