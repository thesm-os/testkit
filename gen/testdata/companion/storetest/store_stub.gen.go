...
var _ companion.Store = (*StoreStub)(nil)

// --- Call types ---

// StoreDeleteCall records one invocation of companion.Store.Delete.
...
// StoreGetCall records one invocation of companion.Store.Get.
// Result fields use the return-value names from the source interface when
// available; unnamed returns produce positional names (Result0, Result1, ...).
...
// StorePutCall records one invocation of companion.Store.Put.
// Result fields use the return-value names from the source interface when
// available; unnamed returns produce positional names (Result0, Result1, ...).
...
func (s *StoreGetStub) Returns(result string, err error) *StoreGetStub {
...
func (s *StorePutStub) Func(fn func(context.Context, string, string) error) *StorePutStub {
	s.fn = fn
	return s
}
...
func StoreStubDelegateTo(impl companion.Store) StoreStubOption {
...
func WithStorePut(fn func(context.Context, string, string) error) StoreStubOption {
	return func(s *StoreStub) { s.OnPut.Func(fn) }
}

// StoreStub is a configurable test double for [companion.Store].
...
func (s *StoreStub) Delete(ctx context.Context, key string) error {
	...
	call := StoreDeleteCall{Ctx: ctx, Key: key}
...
func (s *StoreStub) Get(ctx context.Context, key string) (string, error) {
...
func (s *StoreStub) Put(ctx context.Context, key string, value string) error {
	s.OnPut.SleepLatency()
	call := StorePutCall{Ctx: ctx, Key: key, Value: value}
