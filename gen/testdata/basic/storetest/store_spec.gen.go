...
// Source: basic.go:55-78 (testkit suite Store)
...
//
//	Default subtests: 30 across 7 methods
//	Shapes detected:  Reader (Get), Writer (LegacyPut, Put), Deleter (Delete), Lifecycle (Ping), Unknown (Count, Find)
//	Directives:       bounded (Count: 0..1000), errors (Get→ErrNotFound), deprecated (LegacyPut→PutBatch), timeout (Ping: 5s), nilsafe (Put)
//	Plug-in points:   StoreOnCount, StoreOnDelete, StoreOnFind, StoreOnGet, StoreOnLegacyPut, StoreOnPing, StoreOnPut, StoreOnAll, StoreCustom
...
func runStoreDelete(t *testing.T, factory func() basic.Store, cfg *storeConfig) {
	...
	if len(cfg.onDelete) > 0 {
		t.Run("Delete", func(t *testing.T) {
			...
			dctx := suite.DeleterContext[basic.Store, string]{
				T: t,
				DeleterBindings: bindings.DeleterBindings[basic.Store, string]{
					Factory: prePopFactory,
					Call: func(ctx context.Context, impl basic.Store, k string) error {
...
func runStoreFind(t *testing.T, factory func() basic.Store, cfg *storeConfig) {
	t.Helper()
	...
	t.Run("Find/smoke", func(t *testing.T) {
		...
		defer func() {
			if r := recover(); r != nil {
				t.Skipf("smoke: Find panicked on zero-value args (%v) — supply sample values via StoreOnFind", r)
			}
		}()
		s := factory()
...
func StoreOnDelete(assertions ...suite.DeleterAssertion[basic.Store, string]) StoreOption {
	return func(c *storeConfig) {
		c.onDelete = append(c.onDelete, assertions...)
	}
}

// StoreOnFind adds plug-in assertions for the Find method.
func StoreOnFind(assertions ...func(*testing.T, basic.Store)) StoreOption {
	return func(c *storeConfig) {
...
type storeConfig struct {
	...
	onDelete    []suite.DeleterAssertion[basic.Store, string]
	onFind      []func(*testing.T, basic.Store)
