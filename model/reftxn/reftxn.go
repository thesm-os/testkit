...
// Package reftxn provides the [SnapshotIsolation] reference for
...
var ErrTxClosed = errors.New("reftxn: transaction is closed")
...
// returned by [Tx.Get] for absent keys.
...
func (s *SnapshotIsolation[K, V]) Begin(_ context.Context) (*Tx[K, V], error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// Tx is a snapshot-isolated transaction. Reads come from the
...
func (t *Tx[K, V]) Get(_ context.Context, k K) (V, error) {
	if t.closed {
		var zero V
...
func (t *Tx[K, V]) Put(_ context.Context, k K, v V) error {
	if t.closed {
		return ErrTxClosed
	}
	t.buffer[k] = v
...
func (t *Tx[K, V]) Commit(_ context.Context) error {
	if t.closed {
		return ErrTxClosed
	}
	t.store.mu.Lock()
...
func (t *Tx[K, V]) Rollback(_ context.Context) error {
	if t.closed {
		return ErrTxClosed
	}
	t.closed = true
