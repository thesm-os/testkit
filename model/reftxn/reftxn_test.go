...
func TestSnapshotIsolation(t *testing.T) {
	...
	t.Run("Commit applies buffered writes atomically", func(t *testing.T) {
		t.Parallel()
		s := reftxn.NewSnapshotIsolation[string, int](errNotFound)
	...
	t.Run("operations on closed tx error", func(t *testing.T) {
		...
		testkit.True(t, errors.Is(err, reftxn.ErrTxClosed), "Put errors")
		_, err = tx.Get(t.Context(), "k")
		testkit.True(t, errors.Is(err, reftxn.ErrTxClosed), "Get errors")
		err = tx.Commit(t.Context())
		testkit.True(t, errors.Is(err, reftxn.ErrTxClosed), "double-commit errors")
		err = tx.Rollback(t.Context())
		testkit.True(t, errors.Is(err, reftxn.ErrTxClosed), "rollback after close errors")
	})
}
