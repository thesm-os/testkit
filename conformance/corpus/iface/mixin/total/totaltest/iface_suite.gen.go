...
func runMixedChecks(
	...
	t.Run(label, func(t *testing.T) {
		...
		t.Run("Classify", func(t *testing.T) {
			...
			cfg.run(t, "Classify/an error carries the zero value", "an error carries the zero value", func(tb testing.TB) {
				AssertMixedClassifyZeroOnError(tb, cfg.subject(tb, factory, wrap), cfg.Fixture.InOther)
			})
...
// AssertMixedClassifyZeroOnError asserts an error is accompanied by the zero value.
//
// Fails when: Classify returns a non-zero result alongside a non-nil
// error. A caller who checks the error and a caller who checks the value must
// not disagree about whether the call succeeded.
func AssertMixedClassifyZeroOnError(tb testing.TB, subject total.Mixed, in string) {
	tb.Helper()
	ctx := tb.Context()
	r0, r1 := subject.Classify(ctx, in)
	if r1 == nil {
		tb.Fatalf("Classify succeeded; supply inputs it misses through " +
			"MixedWithFixture")
	}
	{
		var zero string
		testkit.Equal(tb, r0, zero,
			"Classify must return the zero value alongside an error")
	}
}
...
// testkit:provenance cc6c36de7b3a1d6b8c30f1961f406cb62e84e3f716e61ed561749928bf6b6428
