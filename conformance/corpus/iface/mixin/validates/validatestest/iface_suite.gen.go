...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:        validates.Payload{Key: "test-key", Body: "test-body"},
		VOther:   validates.Payload{Key: "other-key", Body: "other-body"},
		Key:      "test-key",
		KeyOther: "other-key",
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject validates.Mixed) validates.Mixed {
...
func runMixedChecks(
	...
	t.Run(label, func(t *testing.T) {
		...
		t.Run("Validate", func(t *testing.T) {
			t.Parallel()
			cfg.run(t, "Validate/smoke", "smoke", func(tb testing.TB) {
				AssertMixedValidateSmoke(tb, cfg.subject(tb, factory, wrap), cfg.Fixture.V)
			})
			for _, c := range cfg.onValidate {
				cfg.run(t, "Validate"+"/"+c.name, c.name, func(tb testing.TB) {
					c.fn(tb, cfg.subject(tb, factory, wrap), cfg.Fixture.V)
				})
			}
		})
...
//
// Fails when: Validate panics. The weakest check in this file and the one
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedValidateSmoke(tb testing.TB, subject validates.Mixed, v validates.Payload) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
			tb.Fatalf("Validate panicked on a derived value (%v); supply one it "+
				"accepts through MixedWithFixture", r)
		}
	}()
	_ = subject.Validate(v)
}
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedReadSmoke(tb testing.TB, subject validates.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedReadCancels(tb testing.TB, subject validates.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedReadHonoursDeadline(tb testing.TB, subject validates.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Read(ctx, key)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedReadToleratesNilContext(tb testing.TB, subject validates.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// testkit:provenance bbb8e71d842ea96f613b62af5c0a7f29f29896bad2cd0d1cdb573883b5be4a8f
