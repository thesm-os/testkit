	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject cacheable.Mixed) cacheable.Mixed {
...
func AssertMixedGetSmoke(tb testing.TB, subject cacheable.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedGetCancels(tb testing.TB, subject cacheable.Mixed, key string) {
	tb.Helper()
...
func AssertMixedGetHonoursDeadline(tb testing.TB, subject cacheable.Mixed, key string) {
	tb.Helper()
...
// request.
func AssertMixedGetToleratesNilContext(tb testing.TB, subject cacheable.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
func AssertMixedGetZeroOnError(tb testing.TB, subject cacheable.Mixed, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// testkit:provenance 35696692eb1a6d02c1f86b1d1c2a8489dbbc8794003f26716e98379c392eb75c
