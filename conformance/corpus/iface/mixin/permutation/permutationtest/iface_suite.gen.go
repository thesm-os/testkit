...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:      permutation.Value{Key: "test-key", Body: "test-body"},
		VOther: permutation.Value{Key: "other-key", Body: "other-body"},
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject permutation.Mixed) permutation.Mixed {
...
func AssertMixedAddSmoke(tb testing.TB, subject permutation.Mixed, v permutation.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedAddCancels(tb testing.TB, subject permutation.Mixed, v permutation.Value) {
	tb.Helper()
...
func AssertMixedAddHonoursDeadline(tb testing.TB, subject permutation.Mixed, v permutation.Value) {
	tb.Helper()
...
// request.
func AssertMixedAddToleratesNilContext(tb testing.TB, subject permutation.Mixed, v permutation.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedItemsSmoke(tb testing.TB, subject permutation.Mixed) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedItemsCancels(tb testing.TB, subject permutation.Mixed) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedItemsHonoursDeadline(tb testing.TB, subject permutation.Mixed) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Items(ctx)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedItemsToleratesNilContext(tb testing.TB, subject permutation.Mixed) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// testkit:provenance 40a213d284c8041ece45a7af7cb21c236aed8b313f9fba5cfcf9dfe82557259c
