...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:        causal.Value{Key: "test-key", Body: "test-body", Rev: 1},
		VOther:   causal.Value{Key: "other-key", Body: "other-body", Rev: 2},
		Key:      "test-key",
		KeyOther: "other-key",
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject causal.Mixed) causal.Mixed {
...
func AssertMixedStoreSmoke(tb testing.TB, subject causal.Mixed, v causal.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedStoreCancels(tb testing.TB, subject causal.Mixed, v causal.Value) {
	tb.Helper()
...
func AssertMixedStoreHonoursDeadline(tb testing.TB, subject causal.Mixed, v causal.Value) {
	tb.Helper()
...
// request.
func AssertMixedStoreToleratesNilContext(tb testing.TB, subject causal.Mixed, v causal.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
func AssertMixedStoreZeroOnError(tb testing.TB, subject causal.Mixed, v causal.Value) {
	tb.Helper()
	ctx := tb.Context()
...
func AssertMixedStoreAnswersWhatItStored(tb testing.TB, subject causal.Mixed, v causal.Value, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedGetSmoke(tb testing.TB, subject causal.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedGetCancels(tb testing.TB, subject causal.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedGetHonoursDeadline(tb testing.TB, subject causal.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Get(ctx, key)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedGetToleratesNilContext(tb testing.TB, subject causal.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// run can reach, and a skip says so where a green line would claim a
// comparison that never happened.
func AssertMixedGetZeroOnError(tb testing.TB, subject causal.Mixed, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// testkit:provenance b49782623cb4896d68f528add9e6cb461c6f1c01a404e2e4f6617e5402155e83
