...
func TestMixedContract(t *testing.T) {
	...
	stickytest.RunMixed(t,
		stickytest.MixedHarness[*stickytest.InMemory]{Name: "in-memory", New: stickytest.NewInMemory},
		stickytest.MixedChecks{
			{
				Method: "Get",
				Name:   "reads-back-what-store-wrote",
				Claim:  "Get returns what Store wrote",
				Run: func(tb testing.TB, s sticky.Mixed, fx stickytest.MixedFixture) {
					tb.Helper()
					written := fx.Value()
