...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:        sticky.Value{Key: "test-key", Body: "test-body"},
		VOther:   sticky.Value{Key: "other-key", Body: "other-body"},
		Key:      "test-key",
		KeyOther: "other-key",
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject sticky.Mixed) sticky.Mixed {
...
func AssertMixedStoreSmoke(tb testing.TB, subject sticky.Mixed, v sticky.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedStoreCancels(tb testing.TB, subject sticky.Mixed, v sticky.Value) {
	tb.Helper()
...
func AssertMixedStoreHonoursDeadline(tb testing.TB, subject sticky.Mixed, v sticky.Value) {
	tb.Helper()
...
// request.
func AssertMixedStoreToleratesNilContext(tb testing.TB, subject sticky.Mixed, v sticky.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedGetSmoke(tb testing.TB, subject sticky.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedGetCancels(tb testing.TB, subject sticky.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedGetHonoursDeadline(tb testing.TB, subject sticky.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Get(ctx, key)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedGetToleratesNilContext(tb testing.TB, subject sticky.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
func AssertMixedGetZeroOnError(tb testing.TB, subject sticky.Mixed, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// testkit:provenance 8bccca769ff75eaa737002dead5b8fd07d8b4ccb0ea4096046343c07e779ef65
