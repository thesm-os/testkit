...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:        pointintime.Value{Key: "test-key", Body: "test-body"},
		VOther:   pointintime.Value{Key: "other-key", Body: "other-body"},
		Key:      "test-key",
		KeyOther: "other-key",
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject pointintime.Mixed) pointintime.Mixed {
...
func AssertMixedStoreSmoke(tb testing.TB, subject pointintime.Mixed, v pointintime.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedStoreCancels(tb testing.TB, subject pointintime.Mixed, v pointintime.Value) {
	tb.Helper()
...
func AssertMixedStoreHonoursDeadline(tb testing.TB, subject pointintime.Mixed, v pointintime.Value) {
	tb.Helper()
...
// request.
func AssertMixedStoreToleratesNilContext(tb testing.TB, subject pointintime.Mixed, v pointintime.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedGetSmoke(tb testing.TB, subject pointintime.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedGetCancels(tb testing.TB, subject pointintime.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedGetHonoursDeadline(tb testing.TB, subject pointintime.Mixed, key string) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Get(ctx, key)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedGetToleratesNilContext(tb testing.TB, subject pointintime.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
func AssertMixedGetZeroOnError(tb testing.TB, subject pointintime.Mixed, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// testkit:provenance ba33b22b49cd6ddf4ccc837838a85b2c9c7d12f35e26186319a4bea2b02fc857
