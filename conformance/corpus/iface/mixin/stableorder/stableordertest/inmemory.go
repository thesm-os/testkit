...
// Package stableordertest holds the generated harness and double for
// [go.thesmos.sh/testkit/conformance/corpus/iface/mixin/stableorder], and the
...
type InMemory struct {
	mu     sync.Mutex
	values map[string]stableorder.Value
}

var _ stableorder.Mixed = (*InMemory)(nil)

// NewInMemory returns an empty collection.
func NewInMemory() *InMemory {
	return &InMemory{values: map[string]stableorder.Value{}}
}
...
func (s *InMemory) Add(ctx context.Context, v stableorder.Value) error {
...
func (s *InMemory) Items(ctx context.Context) ([]stableorder.Value, error) {
	if err := contextErr(ctx); err != nil {
		return nil, err
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]stableorder.Value, 0, len(s.values))
	for _, v := range s.values {
		out = append(out, v)
	}
	slices.SortFunc(out, func(a, b stableorder.Value) int {
...
func contextErr(ctx context.Context) error {
	if ctx == nil {
		return errors.New("stableordertest: nil context")
	}
	return ctx.Err()
}
