...
func DefaultMixedFixture() MixedFixture {
	return MixedFixture{
		V:      overmatch.Value{Key: "test-key", Body: "test-body"},
		VOther: overmatch.Value{Key: "other-key", Body: "other-body"},
	}
}
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject overmatch.Mixed) overmatch.Mixed {
...
func AssertMixedAddSmoke(tb testing.TB, subject overmatch.Mixed, v overmatch.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedAddCancels(tb testing.TB, subject overmatch.Mixed, v overmatch.Value) {
	tb.Helper()
...
func AssertMixedAddHonoursDeadline(tb testing.TB, subject overmatch.Mixed, v overmatch.Value) {
	tb.Helper()
...
// request.
func AssertMixedAddToleratesNilContext(tb testing.TB, subject overmatch.Mixed, v overmatch.Value) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// that catches the most — a method that panics on a derived value is one no
// other check here reaches.
func AssertMixedItemsSmoke(tb testing.TB, subject overmatch.Mixed) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// deadline check. Asserting only that something came back makes the two one
// check written twice.
func AssertMixedItemsCancels(tb testing.TB, subject overmatch.Mixed) {
	tb.Helper()
	ctx, cancel := context.WithCancel(tb.Context())
	cancel()
...
// is read: a generated check that consults the wall clock is one whose subject
// is partly the machine it runs on.
func AssertMixedItemsHonoursDeadline(tb testing.TB, subject overmatch.Mixed) {
	tb.Helper()
	ctx, cancel := context.WithDeadline(tb.Context(), time.Time{})
	defer cancel()
	_, err := subject.Items(ctx)
...
// that forgot one, and a panic turns that into an outage rather than a failed
// request.
func AssertMixedItemsToleratesNilContext(tb testing.TB, subject overmatch.Mixed) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// testkit:provenance fca3d8bd7efd488550a35743cfcb7918adcfeaee45172f13076a6b0538369b4d
