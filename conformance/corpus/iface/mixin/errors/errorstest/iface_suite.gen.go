	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runMixedChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject errors.Mixed) errors.Mixed {
...
func AssertMixedGetSmoke(tb testing.TB, subject errors.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertMixedGetCancels(tb testing.TB, subject errors.Mixed, key string) {
	tb.Helper()
...
func AssertMixedGetHonoursDeadline(tb testing.TB, subject errors.Mixed, key string) {
	tb.Helper()
...
// request.
func AssertMixedGetToleratesNilContext(tb testing.TB, subject errors.Mixed, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
func AssertMixedGetZeroOnError(tb testing.TB, subject errors.Mixed, key string) {
	tb.Helper()
	ctx := tb.Context()
...
// testkit:provenance c04f6398177d272ca4cddc193bc6e706be8022dc8aa47896eb0cfa92ed63a197
