...
//
//   - rate-limit, on Run — write it with ContractOnRun
//
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runContractChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject ratelimit.Contract) ratelimit.Contract {
...
func AssertContractRunSmoke(tb testing.TB, subject ratelimit.Contract, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertContractRunCancels(tb testing.TB, subject ratelimit.Contract, key string) {
	tb.Helper()
...
func AssertContractRunHonoursDeadline(tb testing.TB, subject ratelimit.Contract, key string) {
	tb.Helper()
...
// request.
func AssertContractRunToleratesNilContext(tb testing.TB, subject ratelimit.Contract, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// testkit:provenance 023f41d0ac942ebdcda8b5a81f73952224c40d7faf2313ceeff6e1c607c6981d
