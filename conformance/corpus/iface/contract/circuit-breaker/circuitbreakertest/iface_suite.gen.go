...
//
//   - circuit-breaker, on Run — write it with ContractOnRun
//
	...
	for _, s := range cfg.subjects {
		...
		if !cfg.withoutDouble {
			runContractChecks(t, cfg, s.name+" through the double", s.factory,
				func(tb testing.TB, subject circuitbreaker.Contract) circuitbreaker.Contract {
...
func AssertContractRunSmoke(tb testing.TB, subject circuitbreaker.Contract, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// check written twice.
func AssertContractRunCancels(tb testing.TB, subject circuitbreaker.Contract, key string) {
	tb.Helper()
...
func AssertContractRunHonoursDeadline(tb testing.TB, subject circuitbreaker.Contract, key string) {
	tb.Helper()
...
// request.
func AssertContractRunToleratesNilContext(tb testing.TB, subject circuitbreaker.Contract, key string) {
	tb.Helper()
	defer func() {
		if r := recover(); r != nil {
...
// testkit:provenance b046fe5afceb5b2bbc886ce0edfa745f01adc0c0cc1c14cb04876a881852cf66
