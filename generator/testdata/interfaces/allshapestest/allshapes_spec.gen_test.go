...
// Source: allshapes.go:40-125 (testkit suite -o allshapestest/allshapes_spec.gen_test.go AllShapes)
//
// Directives:
...
// `t.Run(name, ...)` per impl:
//
//	func TestAllShapesContractAcrossImpls(t *testing.T) {
...
func AssertAllShapesContract(t *testing.T, factory AllShapesFactory, opts ...suite.Option) {
	...
	//       4 workers × 10 iterations under -race
	//
	// Sample alignment: PoisonAccessor's contract expects nil on a fresh impl, which most in-mems satisfy naturally. The poisoned-impl extra fires only when WithPoisonedFactory is supplied; that factory must produce an impl whose accessor returns the declared //testkit:errors sentinel.
	...
	//       //testkit:stream-reflects-mutations Stream=All — values written via this method appear in the named stream
	t.Run("Put", func(t *testing.T) {
		...
		suite.AssertStreamReflectsValueWritten[interfaces.AllShapes, interfaces.Record](
			interfaces.Record{ID: "test-id"},
			func(ctx context.Context, impl interfaces.AllShapes, v interfaces.Record) error {
				return impl.Put(ctx, v)
			},
			func(ctx context.Context, impl interfaces.AllShapes) ([]interfaces.Record, error) {
				out := make([]interfaces.Record, 0)
	...
	// Directive contracts (extra subtests below the baseline):
	//   - delete removes
	//       //testkit:delete-removes Reader=Get — a delete of key followed by a read surfaces the missing-key sentinel
	...
	//       //testkit:lifecycle-after-close Reader=Get — calls to Get after this method returns ErrClosed (or equivalent)
	t.Run("Reset", func(t *testing.T) {
		...
		suite.AssertLifecycleAfterClose[interfaces.AllShapes, string, interfaces.Record](
			"test-key",
			...
			func(ctx context.Context, impl interfaces.AllShapes, k string) (interfaces.Record, error) {
	...
	// Directive contracts (extra subtests below the baseline):
	//   - read after write
	//       //testkit:read-after-write Reader=Get — a write of (key, value) followed by a read returns the written value
	t.Run("Set", func(t *testing.T) {
		...
		suite.AssertReadAfterWriteByKey[interfaces.AllShapes, string, interfaces.Record](
			"test-key", interfaces.Record{ID: "test-id"},
			func(ctx context.Context, impl interfaces.AllShapes, k string, v interfaces.Record) error {
